...
test.describe("tests set for auth page", () => {
  ...
  const errorMessage = '//*[@class="error-messages"]/li';
  const errorMessage1SignUp = '//*[@class="error-messages"]/li[1]';
  const errorMessage2SignUp = '//*[@class="error-messages"]/li[2]';
  const profileName = '//*[@href="/@fakeusername4/"]';
  ...
  test(
    ...
    async ({ page }) => {
      ...
      await page.locator(usernameInput).fill("fakeusername4");
      await page
        .locator(emailInput)
        .pressSequentially(fakeEmail, { delay: 100 });
      await page.locator(passwordInput).fill("qwerty");
      await page.locator(signUpBtn).click();
      await expect(page.locator(profileName)).toContainText("fakeusername4");
    }
  );
  ...
  test("DL-T2 login", { tag: ["@regression", "@smoke"] }, async ({ page }) => {
    ...
    await page
      .locator(emailInput)
      .pressSequentially("fellolenitri-3812@yopmail.com", { delay: 100 });
    await page.locator(passwordInput).fill("qwerty");
  ...
  test(
    "DL-T3 login with incorrect credentials",
    { tag: ["@regression", "@smoke"] },
    async ({ page }) => {
      await page.goto(loginPage);
      await page
  ...
  test(
    "DL-T4 Register with existed user",
    { tag: ["@regression", "@smoke"] },
    async ({ page }) => {
      await page.goto(registerPage);
      ...
      await page.locator(passwordInput).fill("qwerty");
      await page.locator(signUpBtn).click();
      await expect(page.locator(errorMessage1SignUp)).toContainText(
        "username is already taken."
      );
      await expect(page.locator(errorMessage2SignUp)).toContainText(
  ...
  test(
    'DL-T4 redirects with "Have an account?" and "Need an account?" links ',
    { tag: ["@regression", "@smoke"] },
    async ({ page }) => {
      const registerPageLink = page.getByRole("link", {
        name: "Need an account?",
      });
      const loginPageLink = page.getByRole("link", {
